...
@app.route("/genereaza-bratara-test")
def genereaza_bratara_test():
    alphabet = "ABCDEFGHJKMNPQRSTUVWXYZ23456789"
    # câte brățări generăm — poți schimba din URL: /genereaza-bratara-test?n=10
    try:
        n = min(int(request.args.get("n", 10)), 50)  # maxim 50 odată
    except (ValueError, TypeError):
        n = 10

    rows = []
    for _ in range(n):
        code = "TEST" + "".join(_secrets.choice(alphabet) for _ in range(4))
        band = Band(code=code, status="unactivated")
        db.session.add(band)
        scan_url = url_for("scan", code=code, _external=True)
        rows.append((code, scan_url))
    db.session.commit()

    items = "".join(
        f"<div style='padding:14px;border-bottom:1px solid #eee'>"
        f"<b>{code}</b><br>"
        f"<a href='{scan_url}' style='color:#2E7D6B;word-break:break-all'>{scan_url}</a>"
        f"</div>"
        for code, scan_url in rows
    )
    return (
        f"<div style='font-family:sans-serif;max-width:600px;margin:40px auto;padding:24px'>"
        f"<h2>{n} brățări de test create ✅</h2>"
        f"<p>Fiecare link poate fi activat separat. Bagă-l într-un generator de QR "
        f"(ex. qrcode-monkey.com) ca să-l scanezi de pe telefon.</p>"
        f"{items}"
        f"</div>"
    )
...
# Creează automat tabelele la pornire dacă nu există (nu șterge date existente).
# Asta permite rularea pe planuri fără acces la Shell.
