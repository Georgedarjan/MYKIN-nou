...
def genereaza_bratara_test():
    ...
    return (
        f"<div style='font-family:sans-serif;max-width:480px;margin:40px auto;"
        f"padding:24px;text-align:center'>"
        f"<h2>Brățară de test creată ✅</h2>"
        f"<p>Cod: <b>{code}</b></p>"
        f"<p>Scaneaz-o (sau apasă link-ul) ca s-o activezi:</p>"
        f"<p><a href='{scan_url}' style='font-size:18px;color:#2E7D6B'>{scan_url}</a></p>"
        f"</div>"
    )
